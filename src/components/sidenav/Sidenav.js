...
class Sidenav extends Component {
  ...
  render() {
    return (<div className="sidebar">
      <div className="sidenav-items">
        {
          sidenavItems.map((sidenavItem, index) => {
            return (<Link to={sidenavItem.link} onClick={this._selectSidenavItem.bind(this, sidenavItem)} key={index} className={classnames("sidenav-item", {
      ...
      <Menu className="theme-menu" label={"Theme"} selected={this.state.selectedTheme} ref={"theme-menu"} onChange={this._switchTheme.bind(this)}>
        {
          themes.map(theme => {
            return <Option onChange={this._switchTheme.bind(this)} value={theme} ref={theme.file} key={theme.file} selected={this.state.selectedTheme.replace('theme-', '') === theme.file.replace('theme-', '')}>
