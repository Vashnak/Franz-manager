        ...
        if (!this.keepMatrix) {
            ...
            this.clusters = this.state.clusters.reduce((prev, next) => {
                ...
                prev.push(next.zookeepers.map(z => {
                    return {cluster: next.name, type: 'zookeeper', id: z.id}
                }));
                return prev;
            }, []);
...
function findFreeNeighbourPosition(strSeed, matrix, x, y) {
    let positions = ["topLeft", "topRight", "right", "bottomRight", "bottomLeft", "left"];
    let freePosition = seededShuffleArray(strSeed, positions).find(pos => {
...
function drawZookeeperIcon(x = 0, y = 0, color = "black") {
    return new Konva.Shape({
        x,
        y,
        fill: color,
...
function seededShuffleArray(str, arr) {
    var rArr = [];
